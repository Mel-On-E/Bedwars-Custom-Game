...
function BedwarsTrader.server_onCreate( self )
...
function BedwarsTrader.sv_save( self )
...
function BedwarsTrader.server_onRefresh( self )
	self:sv_init()
end

function BedwarsTrader.sv_init( self )
...
function BedwarsTrader.sv_vacuumObject( self )
	local container = self.interactable:getContainer( 0 )
...
function BedwarsTrader.client_onCreate( self )
	self:cl_init()
end

function BedwarsTrader.client_onRefresh( self )
	if self.cl then
...
function BedwarsTrader.cl_init( self )
	if self.cl == nil then
		self.cl = {}
	end
...
function BedwarsTrader.client_onDestroy(self)
...
function BedwarsTrader.cl_updateTradeGrid( self )
	self.cl.guiInterface:clearGrid( "TradeGrid" )
	self.cl.guiInterface:addGridItemsFromFile( "TradeGrid", "$CONTENT_DATA/bedwars.json" )
end

function BedwarsTrader.cl_n_addVacuumItem( self, params )
	if self.cl == nil then
		self.cl = {}
	end
	if self.cl.vacuumItems == nil then
...
function BedwarsTrader.client_onClientDataUpdate( self, clientData )
	if self.cl == nil then
		self.cl = {}
	end
...
function BedwarsTrader.client_onInteract( self, character, state )
	if state == true then
...
function BedwarsTrader.cl_onClose( self )
	if self.cl.user then
		self.cl.user.clientPublicData.interactableCameraData = nil
		self.cl.user.character:setLockingInteractable( nil )
		self.cl.user = nil
...
function BedwarsTrader.client_onUpdate( self, dt )
...
function BedwarsTrader.cl_createAnimation( self, name, playTime, nextAnimation, looping, playForward )
	local animation =
	{
		-- Required
		name = name,
...
function BedwarsTrader.cl_setAnimation( self, animation, playProgress )
...
function BedwarsTrader.cl_unsetAnimation( self )
...
function BedwarsTrader.cl_selectAnimation( self )
...
function BedwarsTrader.cl_updateAnimation( self, dt )
