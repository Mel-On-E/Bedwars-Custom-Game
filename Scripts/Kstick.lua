...
local renderablesTp = { "$GAME_DATA/Character/Char_Male/Animations/char_male_tp_sledgehammer.rend",
    "$GAME_DATA/Character/Char_Tools/Char_sledgehammer/char_sledgehammer_tp_animlist.rend" }
...
Kstick.swingCount = 2
Kstick.mayaFrameDuration = 1.0 / 30.0
Kstick.freezeDuration = 0.075

Kstick.swings = { "sledgehammer_attack1", "sledgehammer_attack2" }
Kstick.swingFrames = { 4.2 * Kstick.mayaFrameDuration, 4.2 * Kstick.mayaFrameDuration }
Kstick.swingExits = { "sledgehammer_exit1", "sledgehammer_exit2" }

function Kstick:client_onCreate()
    self.isLocal = self.tool:isLocal()
    self:init()
end

function Kstick:client_onRefresh()
    self:init()
    self:loadAnimations()
end

function Kstick:init()
...
function Kstick:loadAnimations()
...
function Kstick:client_onUpdate(dt)
    ...
    if self.isLocal then
        ...
        if preAnimation ~= self.fpAnimations.currentAnimation then
            ...
            local endedSwing = preAnimation == self.swings[self.currentSwing] and
                self.fpAnimations.currentAnimation == self.swingExits[self.currentSwing]
            ...
            elseif isAnyOf(self.fpAnimations.currentAnimation,
                { "guardInto", "guardIdle", "guardExit", "guardBreak", "guardHit" }) then
                keepBlockSprint = true
            end
...
function Kstick:updateFreezeFrame(state, dt)
...
function Kstick:server_startEvent(params)
...
function Kstick:client_startLocalEvent(params)
    self:client_handleEvent(params)
end

function Kstick:client_handleEvent(params)
...
function Kstick:client_onEquippedUpdate(primaryState, secondaryState)
    ...
    --Start attack?
    self.startedSwinging = (self.startedSwinging or primaryState == sm.tool.interactState.start) and
        primaryState ~= sm.tool.interactState.stop and primaryState ~= sm.tool.interactState.null
    if primaryState == sm.tool.interactState.start or
        (primaryState == sm.tool.interactState.hold and self.startedSwinging) then
...
function Kstick:client_onEquip(animate)
...
function Kstick:client_onUnequip(animate)
